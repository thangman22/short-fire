...
const help = `
Usage: frame-link [command] <options>
...
Examples:
  $ frame-link http://example.com
  $ frame-link http://example.com example
...
if ((!fireLinkConfig['project-id'] || !fireLinkConfig['token'] || !fireLinkConfig['domain']) && !argv['_'][0] === 'init') {
  textBox(chalk.red('• Error') + ' Configulation not found. Please run `frame-link init`')
  process.exit(0)
}
...
async function onInit () {
  ...
  textBox(chalk.green.bold('• Completed') + ' Create configulation Please run `frame-link create [url]`')
}
